"""
Enhanced RRF RAPM Inference using Gemini API
=============================================

This script runs inference using enhanced prompts generated by enhanced_prompt_rrf.py.
It loads prompts from ENHANCED_RRF_*.json files and evaluates with comprehensive metrics.

Key differences from run_prostt5_rapm.py:
- Loads enhanced prompts (better structure, task-specific instructions)
- Otherwise identical evaluation pipeline
- Saves results with "ENHANCED_RRF" prefix for easy comparison

Usage:
    python run_enhanced_rrf.py <task_name> <top_k> [model]

Example:
    python run_enhanced_rrf.py protein_function_OOD 10 gemini-2.5-flash
"""
...
def evaluation(lines, labels, meta_labels, result_file):
    ...
    print('BLEU-2 score:', bleu2 * 100, file=result_file)
    print('BLEU-4 score:', bleu4 * 100, file=result_file)

    meta_bleu2 = corpus_bleu(meta_references, meta_hypotheses, weights=(.5, .5))
    meta_bleu4 = corpus_bleu(meta_references, meta_hypotheses, weights=(.25, .25, .25, .25))
    print('Meta-BLEU-2 score:', meta_bleu2 * 100, file=result_file)
    print('Meta-BLEU-4 score:', meta_bleu4 * 100, file=result_file)
    print('Meta-BLEU-2 score:', meta_bleu2 * 100)
    print('Meta-BLEU-4 score:', meta_bleu4 * 100)

    _meteor_score = np.mean(meteor_scores)
    print('Average Meteor score:', _meteor_score * 100, file=result_file)
...
def api_inference(RAG_prompt, model):
    ...
    generation_config = genai.GenerationConfig(
        temperature=0.7,
        top_p=0.9,
        max_output_tokens=16384,  # Increased from 8192 for better generation
    )
    ...
    try:
        ...
        if candidate.finish_reason == 3:  # SAFETY
            ...
            print("Warning: Response truncated (max tokens reached)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python run_enhanced_rrf.py <task_name> <top_k> [model]")
        print("Example: python run_enhanced_rrf.py protein_function_OOD 10 gemini-2.5-flash")
        sys.exit(1)
    ...
    parent_dir = os.path.dirname(script_dir)  # structural_retrieval/ where prompts are saved
    results_dir = os.path.join(parent_dir, "results")
    os.makedirs(results_dir, exist_ok=True)

    result_file = open(os.path.join(results_dir, "enhanced_rrf_results.txt"), "a+")

    all_input_prompt_len = 0

    # Load enhanced prompts from parent directory
    JSON_PATH = os.path.join(parent_dir, f"ENHANCED_RRF_{now_task}_RAP_Top_{now_k}.json")

    if not os.path.exists(JSON_PATH):
        print(f"Error: Enhanced prompt file {JSON_PATH} not found!")
        print(f"Please run: python enhanced_prompt_rrf.py ../dataset {now_k}")
        sys.exit(1)

    dic = json.load(open(JSON_PATH, "r"))
    infer_numbers = 256  # Evaluate on 256 samples

    print("="*80)
    print(f"Enhanced RRF RAPM Evaluation")
    print(f"Task: {now_task}, Top-K: {now_k}, Model: {model}")
    print(f"Prompt file: ENHANCED_RRF_{now_task}_RAP_Top_{now_k}.json")
    ...
    results_path = os.path.join(results_dir, f"ENHANCED_RRF_{now_task}_{now_k}_results.json")
    ...
    print("="*80)
    print("Enhanced RRF Evaluation complete!")
    print(f"Results saved to: {results_path}")
    print(f"Metrics saved to: {os.path.join(results_dir, 'enhanced_rrf_results.txt')}")
    print("="*80)
    print("\nTo compare with baseline RRF:")
    print(f"  Baseline: {os.path.join(results_dir, 'prostt5_rapm_results.txt')}")
    print(f"  Enhanced: {os.path.join(results_dir, 'enhanced_rrf_results.txt')}")
    print("="*80)
