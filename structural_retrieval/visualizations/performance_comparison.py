#!/usr/bin/env python3
"""
Generate bar graph comparing baseline vs structural retrieval methods
across four datasets (protein_function_OOD, general_function_OOD,
domain_motif_OOD, catalytic_activity_OOD).

Compares three methods:
1. Baseline (no structural embeddings)
2. Hybrid RRF (with structural embeddings)
3. Hybrid Weighted Similarity (alpha=0.7, with structural embeddings)
...
def compute_std_devs(results):
    """
    Compute standard deviation across runs for each task and metric.

    Returns:
        dict: {task_name: {metric_name: std_dev}}
    """
    std_devs = {}
    for task, metrics in results.items():
        std_devs[task] = {}
        for metric, values in metrics.items():
            std_devs[task][metric] = np.std(values, ddof=1) if len(values) > 1 else 0
    return std_devs


def create_comparison_plot(baseline_results, hybrid_rrf_results, hybrid_weighted_results,
                          metric='METEOR', output_path='performance_comparison.png'):
    """
    ...
    Args:
        ...
        hybrid_rrf_results: dict of {task: {metric: [run1, run2, run3]}}
    ...
    # Standard colors: red, blue, green
    color_baseline = 'red'
    color_rrf = 'blue'
    ...
    rrf_means = [np.mean(hybrid_rrf_results[task][metric]) for task in tasks]
    ...
    rrf_stds = [np.std(hybrid_rrf_results[task][metric], ddof=1) for task in tasks]
    ...
    bars2 = ax.bar(x, rrf_means, width, label='Hybrid RRF (Structural)',
                   color=color_rrf, alpha=0.7, yerr=rrf_stds,
                   capsize=5, error_kw={'linewidth': 2, 'elinewidth': 2})
    bars3 = ax.bar(x + width, weighted_means, width,
                   label='Hybrid Weighted (α=0.7, Structural)',
    ...
    add_value_labels(bars2, rrf_means, rrf_stds)
    add_value_labels(bars3, weighted_means, weighted_stds)

    # Customize plot
    ax.set_xlabel('Dataset', fontsize=12, fontweight='bold')
    ax.set_ylabel(f'{metric} Score', fontsize=12, fontweight='bold')
    ax.set_title(f'Performance Comparison: Structural Embeddings Improvement\n({metric} Score, Averaged over 3 runs of 256 samples)',
...
def create_multi_metric_comparison(baseline_results, hybrid_rrf_results, hybrid_weighted_results,
                                   ...
                                   output_path='multi_metric_comparison.png'):
    """
    ...
    for idx, metric in enumerate(metrics):
        ...
        bars2 = ax.bar(x, rrf_means, width, label='Hybrid RRF',
                      color=color_rrf, alpha=0.7, yerr=rrf_stds,
                      capsize=4, error_kw={'linewidth': 1.5, 'elinewidth': 1.5})
        bars3 = ax.bar(x + width, weighted_means, width,
                      label='Hybrid Weighted', color=color_weighted, alpha=0.7,
                      yerr=weighted_stds,
                      capsize=4, error_kw={'linewidth': 1.5, 'elinewidth': 1.5})
...
def print_summary_table(baseline_avg, hybrid_rrf_avg, hybrid_weighted_avg, metric='METEOR'):
    ...
    print(f"{'='*80}")
    print(f"{'Dataset':<25} {'Baseline':<15} {'Hybrid RRF':<15} {'Hybrid Weighted':<15}")
    print(f"{'-'*80}")

    for task in tasks:
        baseline_score = baseline_avg[task][metric]
        rrf_score = hybrid_rrf_avg[task][metric]
        weighted_score = hybrid_weighted_avg[task][metric]

        print(f"{task_labels[task]:<25} {baseline_score:>6.2f} {' '*8} "
              f"{rrf_score:>6.2f} {' '*8} {weighted_score:>6.2f}")
    ...
    print(f"{'-'*80}")
    print(f"{'Dataset':<25} {'Hybrid RRF':<15} {'Hybrid Weighted':<15}")
    print(f"{'-'*80}")

    for task in tasks:
        baseline_score = baseline_avg[task][metric]
        rrf_improvement = ((hybrid_rrf_avg[task][metric] - baseline_score) / baseline_score) * 100
        weighted_improvement = ((hybrid_weighted_avg[task][metric] - baseline_score) / baseline_score) * 100

        print(f"{task_labels[task]:<25} {rrf_improvement:>6.2f}% {' '*8} {weighted_improvement:>6.2f}%")
...
def main():
    ...
    hybrid_rrf_file = project_root / 'structural_retrieval/results/hybrid_rrf_rapm_256_results.txt'
    ...
    print("Parsing hybrid RRF results...")
    hybrid_rrf_results = parse_results_file(hybrid_rrf_file)
    hybrid_rrf_avg = compute_averages(hybrid_rrf_results)
    ...
    create_comparison_plot(
        baseline_results, hybrid_rrf_results, hybrid_weighted_results,
        metric='Meta-BLEU-2',
        output_path=output_dir / 'meta_bleu2_comparison.png'
    )
    ...
    create_comparison_plot(
        baseline_results, hybrid_rrf_results, hybrid_weighted_results,
        metric='Meta-BLEU-4',
        output_path=output_dir / 'meta_bleu4_comparison.png'
    )
    ...
    create_multi_metric_comparison(
        ...
        output_path=output_dir / 'meta_bleu_comparison.png'
    )

    # Print summary tables
    print_summary_table(baseline_avg, hybrid_rrf_avg, hybrid_weighted_avg, metric='Meta-BLEU-2')
    print_summary_table(baseline_avg, hybrid_rrf_avg, hybrid_weighted_avg, metric='Meta-BLEU-4')
